...
:root{color-scheme:dark;font-family:system-ui,sans-serif;background:#a9ddf7;color:#001e60}html,body{width:100%;height:100%;overflow:hidden}body{margin:0;padding-top:.625in;display:flex;flex-direction:column;align-items:center;text-align:center;background:#a9ddf7;color:#001e60;box-sizing:border-box}h1{width:100%;margin:0 0 10px;padding:10px 46px;box-sizing:border-box;background:linear-gradient(180deg,#46515c 0%,#2f3943 45%,#202933 100%);border-bottom:1px solid rgb(255 255 255 / 54%);border-radius:0 0 14px 14px;color:#fff;font-size:clamp(22px,4vh,32px)}p{margin:7px 0}#qr{display:inline-block;margin:auto;background:#fff;padding:10px;border-radius:10px}#qr img,#qr canvas,#qr svg{display:block;width:min(66vw,43vh,300px)!important;height:auto!important}.expires{color:#001e60;font-weight:700}button{font:inherit;font-weight:700;padding:10px 20px;border:1px solid #a9ddf7;border-radius:7px;background:#0053e2;color:#fff}</style></head><body><header><h1>Scan to Create Profile</h1></header><p>Have the new user scan this code with their camera.</p><div id="qr" aria-label="QR code for the new-user invitation"></div><p class="expires">This single-use invitation expires in 15 minutes.</p><form method="post"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="token" value="<?=htmlspecialchars($token,ENT_QUOTES)?>"><button type="submit">Regenerate</button></form><script src="../../vendor/qrcode.min.js"></script><script>
