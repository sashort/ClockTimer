...
:root{color-scheme:dark;font-family:system-ui,sans-serif;background:#001e60;color:#fff}html,body{width:100%;height:100%;overflow:hidden}body{margin:0;display:grid;place-items:center;padding:12px;box-sizing:border-box}main{width:100%;max-width:560px;max-height:100%;overflow:hidden;text-align:center;background:#293746;border:1px solid #a9ddf7;border-radius:14px;padding:clamp(10px,2.5vh,20px);box-sizing:border-box}h1{margin:0 0 6px;font-size:clamp(22px,4vh,32px)}p{margin:7px 0}#qr{display:inline-block;background:#fff;padding:10px;border-radius:10px}#qr img,#qr canvas,#qr svg{display:block;width:min(66vw,43vh,300px)!important;height:auto!important}a{color:#a9ddf7}.expires{color:#ffc220;font-weight:700}button{font:inherit;font-weight:700;padding:10px 20px;border:1px solid #a9ddf7;border-radius:7px;background:#0053e2;color:#fff}</style></head><body><main><h1>Scan to Create Profile</h1><p>Have the new user scan this code with their camera.</p><div id="qr" aria-label="QR code for the new-user invitation"></div><p><a id="invite" href="<?=htmlspecialchars($link,ENT_QUOTES)?>">Open invitation link</a></p><p class="expires">This single-use invitation expires in 15 minutes.</p><form method="post"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="token" value="<?=htmlspecialchars($token,ENT_QUOTES)?>"><button type="submit">Regenerate</button></form></main><script src="../../vendor/qrcode.min.js"></script><script>
