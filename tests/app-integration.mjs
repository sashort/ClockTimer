...
settingsCategories[0].open=true;settingsCategories[0].dispatchEvent(new window.Event('toggle'));
assert.equal(settingsCategories.filter(category=>category.open).length,1);assert.equal(settingsCategories[0].open,true);
assert.equal(graphicalDialog.querySelector('[for="breakBufferColor"]').textContent,'to/from Break');
...
console.log('PASS Clock/Timer settings use a single-open mobile accordion with grouped range colors');
...
assert(previewRanges.some(range=>range.getAttribute('type')==='overtime'));
...
const overtimeToggle=graphicalDialog.querySelector('[name="showOvertime"]');
overtimeToggle.checked=false;overtimeToggle.dispatchEvent(new window.Event('input',{bubbles:true}));
previewRanges=[...preview.querySelectorAll('[data-settings-preview="ranges"] > time-range')];
assert(!previewRanges.some(range=>range.getAttribute('type')==='overtime'));
assert(preview.hasAttribute('hide-overtime'));
...
assert.equal(window.document.querySelector('#graphicalHelpTitle').textContent,'B-Game');
assert.match(window.document.querySelector('#graphicalHelpBody').textContent,/You aimed above 100%/);
assert.match(window.document.querySelector('#graphicalHelpBody').textContent,/miss that goal while you’re still above 100%/);
...
assert.equal(window.document.querySelector('#graphicalHelpBody .settings-help-state-behavior').textContent,'B-Game is shown.');
window.document.querySelector('#graphicalHelpClose').click();await settle();
window.document.querySelector('[data-help-key="overtimeColor"].settings-help-button').click();await settle();
assert.equal(window.document.querySelector('#graphicalHelpTitle').textContent,'Overtime');
assert.match(window.document.querySelector('#graphicalHelpBody').textContent,/shown as Trip time/);
window.document.querySelector('#graphicalHelpClose').click();await settle();
window.document.querySelector('[data-help-key="latencyColor"].settings-help-button').click();await settle();
assert.equal(window.document.querySelector('#graphicalHelpTitle').textContent,'Late Start');
assert.equal(window.document.querySelector('#graphicalHelpBody .settings-help-late').textContent,'LATE!');
assert.match(window.document.querySelector('#graphicalHelpBody').textContent,/Late Start continues until you resume your trip/);
console.log('PASS B-Game, Overtime, and Late Start help use the intended user-facing descriptions');
