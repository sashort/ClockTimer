...
$short=trip_edit_apply($events,['operation'=>'add-entry','entry'=>['type'=>'break','breakType'=>'short','start'=>'2026-09-18T12:10:00Z','end'=>'2026-09-18T12:12:00Z','length'=>'0:02:00']]);$shortStart=array_values(array_filter($short,fn($event)=>$event['event']==='interval.started'&&($event['value']['attributes']['breakType']??null)==='short'));check(count($shortStart)===1,'adding supports a Short Break entry');
$approved=trip_edit_apply($events,['operation'=>'add-entry','entry'=>['type'=>'down','start'=>'2026-09-18T12:10:00Z','end'=>'2026-09-18T12:12:00Z','approvedTime'=>'0:01:00']]);check(trip_edit_aggregate($approved)['counted']===1020000,'approved Down time overrides its actual duration');
$replacement=$events;$replacement[1]['timestamp']='2026-09-18 12:04:00.000';$plan=trip_edit_persistence_plan($events,$replacement);check($plan['keep']===[1,3,4]&&$plan['delete']===[2]&&count($plan['insert'])===1&&$plan['insert'][0]['id']===null,'immutable event edits are planned as delete and insert');
$earlyBreak=$events;$earlyBreak[2]['timestamp']='2026-09-18 12:05:10.000';check(trip_edit_aggregate($earlyBreak)['counted']===1080000,'ending a break early preserves its scheduled allowance');
