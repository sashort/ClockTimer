(() => {
    ...
    class TripLog {
        ...
        trip(trip) {
            ...
            if(this.editing.has(trip.id)){const done=node('button','Done');done.type='button';done.addEventListener('click',()=>this.finishEntryEdit(trip,done));header.append(done);entries.append(header,node('p','Tap a time, interval type, or approved time to edit it. Long-press an entry to show its remove button and, where allowed, insert before it.'));}else entries.append(header);
            ...
            for(const event of visible){const eventKey=String(event.id),started=event.event==='interval.ended'?events.find(e=>e.event==='interval.started'&&e.value.intervalKey===event.value.intervalKey):event,ended=started?.event==='interval.started'?events.find(e=>e.event==='interval.ended'&&e.value.intervalKey===started.value.intervalKey):null;
                ...
                if(this.editing.has(trip.id)){
                    ...
                    if(removableAction&&this.deleteVisible.has(deleteKey)){const remove=node('button','×','trip-log-entry-remove');remove.type='button';const boundaryLabel=event.event==='trip.started'?'Trip Started':event.event==='trip.stopped'?'Trip Ended':String(started?.value?.type||'Interval');remove.setAttribute('aria-label',`Remove ${boundaryLabel}`);remove.addEventListener('click',()=>Promise.resolve(tripBoundary?this.deleteTrip(trip):this.deleteEntry(trip,started)).catch(error=>this.error(error)));row.classList.add('has-entry-remove');row.append(remove);}
        ...
        newEntryRows(trip,draft) {
            ...
            for(const [value,label] of [['break','Break'],['short-break','Short Break'],['lunch','Lunch'],['down','Down']]){const option=node('option',label);option.value=value;type.append(option);}
            ...
            const commit=async()=>{if(!draft.start||!draft.end||!draft.type)return;const key=`draft-${Date.now()}-${Math.random().toString(16).slice(2)}`,effectiveType=draft.type==='short-break'?'break':draft.type,attributes=effectiveType==='break'?{breakType:draft.type==='short-break'?'short':'break'}:{},approvedTime=effectiveType==='down'?(draft.approvedTime||duration(Date.parse(draft.end)-Date.parse(draft.start))):undefined;trip.events.push({id:key,event:'interval.started',timestamp:draft.start,value:{type:effectiveType,length:effectiveType==='down'?null:duration(Date.parse(draft.end)-Date.parse(draft.start)),approvedTime,attributes,intervalKey:key},_draft:true},{id:`${key}-end`,event:'interval.ended',timestamp:draft.end,value:{intervalKey:key},_draft:true});this.newEntries.delete(String(trip.id));this.rerender();};
        ...
        async editEntryName(trip,event,ended,button,addButton) {
            ...
            select.addEventListener('change',async()=>{if(event.event!=='interval.started'){restore();return;}const current=trip.events.find(candidate=>String(candidate.id)===String(event.id));if(!current){restore();this.error(new Error('Entry changed. Reopen the trip.'));return;}current.value.type=select.value==='short-break'?'break':select.value;if(current.value.type==='break')current.value.attributes={...(current.value.attributes||{}),breakType:select.value==='short-break'?'short':'break'};else if(current.value.attributes)delete current.value.attributes.breakType;this.root.querySelector(':scope>.trip-log-error')?.remove();restore();});
