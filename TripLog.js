(() => {
    ...
    class TripLog {
        constructor(root,options) {this.root=root;this.options=options;this.expanded=new Map();this.editing=new Set();this.entrySessions=new Map();this.addBefore=new Map();this.newEntries=new Map();this.deleteVisible=new Set();this.editor=null;this.settingsVisible=false;}
        ...
        trip(trip) {
            ...
            const tripKey=String(trip.id),draft=this.newEntries.get(tripKey);
            const addButton=()=>{const add=node('button','+ Add Entry','trip-log-add');add.type='button';add.addEventListener('click',()=>{this.newEntries.set(tripKey,{});this.rerender();});return add;};
            for(const event of visible){const eventKey=String(event.id),started=event.event==='interval.ended'?events.find(e=>e.event==='interval.started'&&e.value.intervalKey===event.value.intervalKey):event,ended=started?.event==='interval.started'?events.find(e=>e.event==='interval.ended'&&e.value.intervalKey===started.value.intervalKey):null;
                const label=this.entryLabel(event,started,ended,events);
                ...
                if(this.editing.has(trip.id)){
                    ...
                    const editableType=event.event==='interval.started'&&['break','lunch','down'].includes(String(event.value?.type||'').toLowerCase());const name=node(editableType?'button':'span',label,'trip-log-entry-name');if(editableType){name.type='button';name.addEventListener('click',()=>{if(suppressClick){suppressClick=false;return;}this.editEntryName(trip,event,ended,name,entries.querySelector('.trip-log-add')).catch(e=>this.error(e));});}row.append(time,name);
                    ...
                    const intervalAction=['interval.started','interval.ended'].includes(event.event),deleteKey=`${tripKey}:${event.value?.intervalKey}`;
                    ...
                    if(intervalAction){let gesture,timer;row.addEventListener('pointerdown',pointer=>{if(pointer.target.closest('button,select'))return;pointer.preventDefault();gesture={x:pointer.clientX,y:pointer.clientY};timer=setTimeout(()=>{timer=undefined;suppressClick=true;this.deleteVisible.add(deleteKey);this.rerender();},550);});row.addEventListener('pointermove',pointer=>{if(gesture&&(Math.abs(pointer.clientX-gesture.x)>10||Math.abs(pointer.clientY-gesture.y)>10)){clearTimeout(timer);timer=undefined;}});row.addEventListener('pointerup',()=>{clearTimeout(timer);timer=undefined;gesture=undefined;});row.addEventListener('pointercancel',()=>{clearTimeout(timer);timer=undefined;gesture=undefined;});row.addEventListener('contextmenu',pointer=>pointer.preventDefault());}
                }else {row.append(node('span',fmt.format(new Date(iso(event.timestamp)))),node('span',label));if(event.event==='interval.started'&&String(event.value?.type||'').toLowerCase()==='down'){row.append(node('span',`Approved: ${this.approvedTime(event,ended,events)||'---'}`,'trip-log-entry-approved-value'));row.classList.add('has-approved-time');}}entries.append(row);
            }
            if(this.editing.has(trip.id)){if(draft)entries.append(this.newEntryRows(trip,draft));else entries.append(addButton());}
