...
CREATE TABLE IF NOT EXISTS `speech_model_components` (
    ...
    CONSTRAINT `fk_speech_model_root`
        ...
        ON UPDATE RESTRICT ON DELETE RESTRICT,
    CONSTRAINT `chk_speech_model_enabled`
        CHECK (`enabled` IN (0, 1)),
    CONSTRAINT `chk_speech_model_shape`
        CHECK (
            (
...
CREATE TABLE IF NOT EXISTS `speech_training_contributions` (
    ...
    KEY `idx_speech_training_variant` (`phrase_id`, `active`, `mapping_hash`),
...
CREATE TABLE IF NOT EXISTS `speech_corrections` (
    ...
    KEY `idx_speech_correction_runtime` (`enabled`, `mapping_hash`),
    CONSTRAINT `fk_speech_correction_phrase`
        FOREIGN KEY (`phrase_id`) REFERENCES `speech_model_phrases` (`id`)
        ON UPDATE RESTRICT ON DELETE CASCADE,
    CONSTRAINT `chk_speech_correction_enabled`
        CHECK (`enabled` IN (0, 1)),
    CONSTRAINT `chk_speech_correction_occurrences`
        CHECK (`occurrences` > 0),
    CONSTRAINT `chk_speech_correction_contributors`
...
DROP TRIGGER IF EXISTS `audit_speech_training_contribution_add`;
DROP TRIGGER IF EXISTS `audit_speech_training_contribution_edit`;
DROP TRIGGER IF EXISTS `audit_speech_training_contribution_delete`;
DROP TRIGGER IF EXISTS `protect_speech_training_audit_update`;
DROP TRIGGER IF EXISTS `protect_speech_training_audit_delete`;
